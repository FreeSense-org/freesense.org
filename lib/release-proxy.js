const SCHEMA = 'freesense.download/v4';
const V3_SCHEMA = 'freesense.download/v3';
...
function validRelease(value, channel) {
  ...
  if (value?.schema_version === SCHEMA) return validV4Release(value, channel);
  if (value === null
      || typeof value !== 'object'
      || ![V2_SCHEMA, V3_SCHEMA].includes(value.schema_version)
      ...
      || ![1, 3, 5].includes(value.artifacts.length)
  ...
  const architecture = value.schema_version === V3_SCHEMA ? value.architecture : 'amd64';
  const packageArch = value.schema_version === V3_SCHEMA ? value.package_arch : 'amd64';
  if (!((architecture === 'amd64' && packageArch === 'amd64')
      || (architecture === 'arm64' && packageArch === 'aarch64'))) return false;
  if (value.schema_version === V3_SCHEMA
  ...
  const installerOnly = value.schema_version === V3_SCHEMA
    && value.artifacts.length === 1
    && value.capabilities.cloud_init === false;
  if (value.artifacts.length === 1 && !installerOnly) return false;
  const expected = new Set([`installer:none:${installerFormat}`]);
  if (!installerOnly) {
    expected.add('cloud:ufs:qcow2');
    expected.add('cloud:ufs:raw');
  }
...
function validV4Release(value, channel) {
  if (value === null || typeof value !== 'object' || value.channel !== channel
      || !VERSION.test(value.version) || !SHA256.test(value.bundle_fingerprint)
      || !SHA256.test(value.system) || !Number.isInteger(value.generation)
      || value.generation <= 0 || !Array.isArray(value.artifacts)
      || value.artifacts.length === 0 || typeof value.published_at !== 'string'
      || !validChanges(value.changes) || !validReleaseNotes(value.release_notes)) return false;
  if (!((value.architecture === 'amd64' && value.package_arch === 'amd64')
      || (value.architecture === 'arm64' && value.package_arch === 'aarch64'))) return false;
  const releaseId = channel === 'stable' ? value.version : `${value.version}-g${value.generation}`;
  const download = `https://downloads.freesense.org/v1/releases/${channel}/${releaseId}`;
  if (value.release_id !== releaseId || value.artifacts[0]?.kind !== 'installer') return false;
  const fingerprints = new Set();
  for (const artifact of value.artifacts) {
    if (artifact === null || typeof artifact !== 'object'
        || !['installer', 'cloud', 'appliance'].includes(artifact.kind)
        || typeof artifact.platform !== 'string' || !Array.isArray(artifact.target_models)
        || ![null, 'ufs', 'zfs'].includes(artifact.filesystem)
        || !['iso', 'img', 'qcow2', 'raw'].includes(artifact.format)
        || !['none', 'xz'].includes(artifact.compression)
        || !['gpt', 'mbr'].includes(artifact.partition_scheme)
        || !Array.isArray(artifact.firmware) || artifact.capabilities === null
        || typeof artifact.capabilities !== 'object' || artifact.boot_inputs === null
        || typeof artifact.boot_inputs !== 'object'
        || !SHA256.test(artifact.artifact_fingerprint)
        || !SHA256.test(artifact.sha256) || !Number.isInteger(artifact.size)
        || artifact.size <= 0 || !['unverified', 'verified'].includes(artifact.hardware_verification)
        || artifact.url !== `${download}/${artifact.file}`
        || typeof artifact.marker_url !== 'string') return false;
    fingerprints.add(artifact.artifact_fingerprint);
    if (artifact.kind === 'appliance' && (artifact.filesystem !== 'ufs'
        || artifact.format !== 'img' || artifact.compression !== 'xz'
        || artifact.partition_scheme !== 'mbr' || artifact.target_models.length === 0)) return false;
  }
  const provenance = value.provenance;
  return provenance !== null && typeof provenance === 'object'
    && ['source', 'ports', 'os_definition', 'freebsd'].every((name) => SHA.test(provenance[name]))
    && (channel === 'stable' ? value.support_tier === 'supported' : value.support_tier === 'development');
}
...
function createReleaseHandler(channel) {
  ...
  return async function onRequest(context) {
    ...
    try {
      ...
      const actualArchitecture = [SCHEMA, V3_SCHEMA].includes(release.schema_version) ? release.architecture : 'amd64';
...
export {
  LEGACY_SCHEMA, RELEASE_NOTES_SCHEMA, SCHEMA, V2_SCHEMA, V3_SCHEMA, createReleaseHandler, validRelease,
};
