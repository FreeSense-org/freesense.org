const SCHEMA = 'freesense.download/v2';
...
function validRelease(value, channel) {
  ...
  const expected = new Set(['installer:iso', 'cloud:qcow2', 'cloud:raw']);
  for (const artifact of value.artifacts) {
    if (artifact === null || typeof artifact !== 'object'
        || !expected.delete(`${artifact.kind}:${artifact.format}`)
    ...
    if (artifact.kind === 'cloud'
        && (artifact.filesystem !== 'ufs' || artifact.compression !== 'xz'
          || artifact.virtual_size !== 16 * 1024 ** 3)) return false;
  }
...
export { LEGACY_SCHEMA, SCHEMA, createReleaseHandler, validRelease };
